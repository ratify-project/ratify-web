...
| AUTH_DENIED                  | auth denied                  | The authentication to required resource is denied. Please validate the credentials or configuration and check the detailed error. For auth configuration in the registry store, please refer to [registry store credential providers](../plugins/store/oras.md#credential-providers). |
...
| NO_MATCHING_CREDENTIAL | no matching credential | Please verify the credentials are configured for the registry store. Please refer to [registry store credential providers](../plugins/store/oras.md#credential-providers) for more details.                                   |
...
| Code                        | Description                | Trouble Shooting Guide                                                                                                                                                                                                   |
| --------------------------- | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| KEY_VAULT_OPERATION_FAILURE | key vault operation failed | Please validate correct key vault configuration is provided or check the error details for further investigation. Review the Azure Key Vault key provider configuration in the [Configuration](../ratify-configuration.mdx#azure-key-vault-provider) guide. |
