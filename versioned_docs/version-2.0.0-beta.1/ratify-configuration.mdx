...
- **`certificates`**: List of certificates used for verification
  ...
  - **`inline`**: Inline key provider. Provide the PEM-encoded certificate under `inline.certs` (a string).
...
:::caution
The `filesystem-oci-store` is not functional when configured through the Executor CRD in v2.0.0-beta.1 (its parameters fail to decode). Use `registry-store` instead. This section will be restored once the issue is resolved.
:::
...
1. The image is signed by the notation-verifier.
2. AND the image is additionally signed by at least one of two Cosign identities (for example, two different signing teams).

This is useful when you want to combine multiple mandatory signatures with an "any-of" group.
  ...
  parameters:
    policy:
      rules:
        - verifierName: notation-verifier
        - rules:
            - verifierName: cosign-verifier-team-a
            - verifierName: cosign-verifier-team-b
          threshold: 1
      # outer threshold unset => both top-level rules must pass
```
