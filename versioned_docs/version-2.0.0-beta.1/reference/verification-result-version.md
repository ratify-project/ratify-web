...
- [Verification Response Versions](#verification-response-versions)
...
# Verification Response Versions
Ratify has historically supported multiple policy providers that generated different Verification Response formats. Earlier config-based policies produced Verification Response version 0.x, while later providers use version 1.x. The primary difference between these versions is that 0.x was not well designed to handle nested verification and multiple verifiers for the same artifact.
