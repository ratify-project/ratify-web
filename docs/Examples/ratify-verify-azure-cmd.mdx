...
> Note: Azure Key Vault integration currently only support self signed certificate, if you are working with a certificate chain, please specify the public root certificate value inline using the [inline certificate provider](../references/crds/certificate-stores.mdx#inline-certificate-provider).
